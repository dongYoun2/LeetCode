...
# 20 min
# solved using the rotation trick: right rotation by k == the last k nodes move to the front
...
# It wasn't too difficult as long as I could find the k-th node from the end of the list. I learned the technique where you use two pointers (`ahead` and `behind`) to find the k-th node from the end from the problem "19. Remove Nth Node From End of List". One point to consider is that since `k` can be greater than the length of the list, I need to find the length of the list first and then take `k` modulo the length of the list.
