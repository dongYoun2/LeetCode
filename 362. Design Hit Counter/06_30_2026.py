...
# cf.) i noticed that since the `timestamp` is always non-decreasing, we can simply leverage the length of the list to find the index of the given `timestamp`. so, we can simply replace `to_idx = bisect.bisect(self.hits, timestamp, key=lambda e: e[0])` with `to_idx = len(self.hits)`, and thus, performs only one binary search instead of two. submission: https://leetcode.com/problems/design-hit-counter/submissions/2053616738/
