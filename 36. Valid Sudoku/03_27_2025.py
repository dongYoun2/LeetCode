...
# runtime: 8 ms (beats 12.07%), memory: 17.69 MB (beats 100%)
# 29 min
...
# It was a pretty interesting question. I knew how to transpose a 2D matrix with pure Python. However, it was a bit confusing to map each 3x3 sub-box index to the original index. I could come up with the general index conversion formula by thinking carefully about one example. Each sub-box is represented via (y, x), and each element in the sub-box is represented via (i, j). In fact, this reminds me of the CUDA parallel programming during the MPC course:
# 1. Mapping to the original board
# The position (3*y + i, 3*x + j) is like computing a global index from a block index and a thread index (block * size + local). Here (y,x) identifies the sub-box (block) and (i,j) identifies the position inside the box (thread).
# 2. Mapping to the converted board
# The position (3*y + x, 3*i + j) flattens both the sub-box index (y,x) and the inner index (i,j) into linear indices. This is similar to converting a 2D index (row, col) into a linear index using row * width + col.
